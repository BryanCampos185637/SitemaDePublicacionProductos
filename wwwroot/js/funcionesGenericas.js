...
function cambiarEstadoCheck() {
    //quita los check
    var paginasCheck = document.getElementsByClassName("paginasCheck");
    for (var i = 0; i < paginasCheck.length; i++) {
        paginasCheck[i].checked = false;
    }
}
...
function validarContraseña(inputContraseña, mensaje) {
    ...
    if (!(mayusculas.test(contraseñaEvaluar) && caracteresEspeciales.test(contraseñaEvaluar) && numeros.test(contraseñaEvaluar)
        ...
        mensaje.innerHTML = '<span class="text-primary">Contraseña insegura: debe contener mayusculas, minusculas, numeros y caracteres especiales[C@ntras3ña20]</span>';
        respuesta = false;
    }
    return respuesta;
}
...
//generador de sugerencias para nombre de usuario
    ...
    var mensaje = "";
    mensaje += "<span class='text-primary'>El usuario " + $("#txtUsuario").val() + " ya esta en uso.</span><br>";
    if (nrandom < 300) {
        mensaje += "<span class='text-primary'>Prueba con " + $("#txtUsuario").val() + Math.floor(nrandom) + "</span>";
    } else if (nrandom > 300 && nrandom < 500) {
        mensaje += "<span class='text-primary'>Prueba con " + nombre.replace(" ", "_") + Math.floor(nrandom) + "</span>";
    } else if (nrandom > 500 && nrandom < 750)
    {
        mensaje += "<span class='text-primary'>Prueba con " + apellido.replace(" ", "@") + Math.floor(nrandom) + "</span>";
    }
    else if (nrandom > 750) {
        mensaje += "<span class='text-primary'>Prueba con " + apellido.replace(" ", ".") + "@" + nombre.replace(" ","."); + "</span>";
    }
...
function hacerScrollHaciaArriba() {
    $("body,html").animate({
        scrollTop: "0px"
    });
}
